    ...
    if (typeof window._gPlayerAudioCtx == 'undefined') {	// cannot be instantiated 2x (so make it global)
        ...
        try {
            if ('AudioContext' in window) {
                window._gPlayerAudioCtx = Pizzicato.context;//new AudioContext();
            } else if ('webkitAudioContext' in window) {
                window._gPlayerAudioCtx = Pizzicato.context;//new webkitAudioContext();		// legacy stuff
            } else {
                alert(errText + e);
            }
        } catch (e) {
...
EmsHEAP16BackendAdapter = (function () {
    var $this = function (backend, channels) {
		...
		*/
        ...
        initWebAudio: function () {
            if (typeof this._bufferSource != 'undefined') {
                ...
                var reverb2 = new Pizzicato.Effects.Convolver({
                    impulse: 'https://rawgit.com/alemangui/pizzicato/gh-pages/audio/scala-milan.wav',
                    mix: 0.3
                }, function () {
                ...
                compressor.connect(reverb2);
                reverb2.connect(reverb);
                ...
                if (this._spectrumEnabled) {
                    reverb.connect(this._analyzerNode);
                    this._analyzerNode.connect(window._gPlayerAudioCtx.destination);
                } else {
                    reverb.connect(window._gPlayerAudioCtx.destination);
                }
            }
        },
